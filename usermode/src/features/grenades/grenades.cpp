...
bool f::grenades::get_smoke(c_smoke_grenade* smoke)
{
    ...
    const auto owner_handle = smoke->m_hOwnerEntity();
    const auto owner_entry_idx = owner_handle.is_valid() ? owner_handle.get_entry_idx() : ENT_ENTRY_MASK;
    ...
    m_grenade_data = {
        ...
        {"m_y", det_pos.m_y},
        {"m_z", det_pos.m_z},
        {"m_owner_entry_idx", owner_entry_idx}
    };
...
bool f::grenades::get_molo(c_molo_grenade* molo)
{
    ...
    const auto owner_handle = molo->m_hOwnerEntity();
    const auto owner_entry_idx = owner_handle.is_valid() ? owner_handle.get_entry_idx() : ENT_ENTRY_MASK;
...
bool f::grenades::get_thrown(c_base_grenade* nade)
{
    ...
    const auto owner_handle = nade->m_hOwnerEntity();
    const auto owner_entry_idx = owner_handle.is_valid() ? owner_handle.get_entry_idx() : ENT_ENTRY_MASK;
