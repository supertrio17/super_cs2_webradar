...
c_cs_player_controller* c_cs_player_controller::get_local_player_controller()
{
    ...
    {
        const auto local_controller = m_memory->read_t<c_cs_player_controller*>(client_base.value() + dump_a2x::offsets::client_dll::dw_local_player_controller);
        if (local_controller)
            return local_controller;
    }

    const auto pattern_offset = m_memory->find_pattern(CLIENT_DLL, GET_LOCAL_PLAYER_CONTROLLER);
    if (!pattern_offset.has_value())
        return {};

    const auto local_controller_ptr = pattern_offset.value().rip().as<void*>();
    if (!local_controller_ptr)
        return {};

    return m_memory->read_t<c_cs_player_controller*>(local_controller_ptr);
}
